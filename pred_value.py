...
def show_prognoz_value(): 
    st.title('Прогноз на основе введенных данных') 
    global prognoz
    buffer = io.StringIO()
...
# Создание DataFrame с реальными и прогнозируемыми значениями
    ...
    education_level = st.slider('Уровень образования', min_value=0, max_value=3, value=1)
    job_title = st.slider('Название должности', min_value=0, max_value=190, value=1)
    years_experience = st.slider('Количество лет опыта', min_value=0, max_value=34, value=1)
   
    if st.button('Прогнозировать'):
        prediction = predict_salary(education_level, job_title, years_experience)
        st.write(f'Прогнозируемая годовая зарплата: ${int(prediction[0])}')
...
# Вывод элементов словаря
    for key, value in data2.items():
        st.write(f"--------------------------------------")
        st.write(f"Значение: {key}, Должность: {value}")


    for key, value in data3.items():
        st.write(f"--------------------------------------")
        st.write(f"Значение: {key}, Уровень образования: {value}")
